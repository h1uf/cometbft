...
localnet-start: localnet-stop build-docker-localnode
	@if ! [ -f build/node0/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/cometbft:Z cometbft/localnode testnet --v 6 --config /etc/cometbft/config-template.toml --o . --starting-ip-address 192.167.10.5; fi
	docker compose up -d
